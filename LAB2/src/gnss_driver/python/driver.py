...
def gps_publisher():
    ...
    rate=rospy.Rate(1)
    serial_data=serial.Serial(port,57600)
    
    while not rospy.is_shutdown():
        line=serial_data.readline()
        decoded=line.decode('ASCII')
        splitted=decoded.split(',')
        if splitted[0].find('GNGGA')!= -1:
################ Time MANIPULATION #######################            
...
############### LONGI MANIPULATION #######################
            ...
            Zone=int(utm_data[2])
            Letter=utm_data[3]
            fix_quality_added=int(splitted[6])
...
################ Putting in msg ############################
            ...
            gps_message.fix_quality=fix_quality_added
            ...
            rospy.loginfo(f"fix quality:{fix_quality_added}")
